...
## Why Capsolver (Turnstile) Can Still Fail (and what we do about it)

Capsolver **can** solve Cloudflare Turnstile, but the token can be rejected if:

1. **User-Agent mismatch** – When Capsolver returns a `userAgent`, we set `page.setExtraHTTPHeaders({ 'User-Agent': result.userAgent })` after each solve so the next request matches.
2. **IP mismatch** – The token can be tied to the IP that solved it. We pass `MISSINGMONEY_PROXY_URL` into the Capsolver task as a `proxy` string when possible; if Capsolver rejects proxy on `AntiTurnstileTaskProxyLess`, we retry without proxy (their docs primarily document proxyless Turnstile).
3. **Datacenter IP blocking** – Missing Money may block or throttle datacenter IPs. A residential proxy for Playwright improves success.
